...
const HorizontalVideo = () => {
  ...
  const [_videoId, setVideoId] = useState<string | null>(null); // `_` kaldırıldı
  ...
  const handleGenerateVideo = async () => {
   ...
   // ✅ Kelime bazlı ve karakter bazlı süre sınırı
const maxWords = videoOrientation === "Horizontal" ? 75 : 25; // Horizontal: 30 saniye, Vertical: 10 saniye
const maxCharacters = videoOrientation === "Horizontal" ? 300 : 100; // Yedek karakter sınırı
const wordCount = script.trim().split(/\s+/).length; // Kelime sayısını hesapla
const charCount = script.length; // Karakter sayısını hesapla
...
if (wordCount > maxWords || charCount > maxCharacters) {
    ...
    try {
      const response = await fetch("https://video-generator.xyz/generate-video",{
        ...
        body: JSON.stringify({
          ...
          voiceType,
          videoOrientation,
          addWatermark: true, // ✅ Backend'e watermark eklemesi gerektiğini bildiriyoruz
        }),
      });
  ...
  return (
    <div 
      ...
      style={{ backgroundImage: "url('https://xqgftsgrnwtzdsuziqsj.supabase.co/storage/v1/object/public/bg/background3.jpg')" }}
    >
      ...
      <div className="relative z-10 bg-white/90 backdrop-blur-md p-8 rounded-lg shadow-lg w-[90%] max-w-md max-h-screen overflow-y-auto mt-[100px]">
        ...
        <div className="mt-3">
          ...
          <input
            ...
            onChange={handleFileUpload} // ✅ Burada çağırılmalı
          />
        </div>
